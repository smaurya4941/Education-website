    ...
    listenClick('.change-job-stage', function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                let current_stage = data.current_stage;
                let firstStage = Object.keys(data.job_stages)[0] ?? '';
                if (current_stage == null || current_stage === '') {
                ...
                $.each(data.job_stages, function (key, val) {
                    jobStageAttr.append(`<option value="${key}">${val}</option>`);
                });
