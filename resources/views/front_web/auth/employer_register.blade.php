...
<div class="bw-auth-container">
    <div class="bw-auth-card">
        ...
        <div class="bw-auth-tabs">
            <a href="{{ route('candidate.register') }}" class="bw-auth-tab">{{ __('web.register_menu.candidate') }}</a>
            <a href="{{ route('employer.register') }}" class="bw-auth-tab active">{{ __('web.register_menu.employer') }}</a>
        </div>
        ...
        <form method="POST" action="{{ route('front.login') }}" id="addEmployerNewForm">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="bw-input-group">
                        <label for="employerFirstName" class="bw-label">{{ __('web.common.name') }} <span class="text-danger">*</span></label>
                        <input type="text" class="bw-input" name="first_name" id="employerFirstName" placeholder="{{ __('web.register_menu.enter_first_name') }}" required>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="bw-input-group">
                        <label for="employerEmail" class="bw-label">{{ __('web.common.email') }} <span class="text-danger">*</span></label>
                        <input type="email" class="bw-input" name="email" id="employerEmail" placeholder="@lang('web.register_menu.enter_email_address')" required>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="bw-input-group" style="position: relative;">
                        <label for="employerPassword" class="bw-label">{{ __('web.common.password') }} <span class="text-danger">*</span></label>
                        <input type="password" class="bw-input" name="password" id="employerPassword" placeholder="{{ __('web.register_menu.enter_password') }}" required onkeypress="return avoidSpace(event)">
                ...
                <div class="col-md-6">
                    <div class="bw-input-group" style="position: relative;">
                        <label for="employerConfirmPassword" class="bw-label">{{ __('web.common.confirm_password') }} <span class="text-danger">*</span></label>
                        <input type="password" class="bw-input" name="password_confirmation" id="employerConfirmPassword" placeholder="{{ __('web.register_menu.confirm_password') }}" required onkeypress="return avoidSpace(event)">
                        <span class="position-absolute d-flex align-items-center top-50 translate-middle-y {{ getFrontSelectLanguage() == 'ar' ? 'start-0 ms-3' : 'end-0 me-3' }} input-icon input-password-hide cursor-pointer text-gray-600 change-type change-type-register" style="margin-top: 14px;">
                            <i class="fas fa-eye-slash"></i>
                        </span>
                    </div>
            ...
            <button type="submit" class="bw-btn mb-4" id="btnEmployerSave" data-loading-text="<span class='spinner-border spinner-border-sm'></span> {{ __('messages.common.process') }}">
                {{ __('web.register_menu.create_account') }}
            </button>
            
            <div class="text-center mb-4" style="font-size: 14px; color: #6c6a77;">
                {{ __('Already have an account') }} 
                <a href="{{ route('front.employee.login') }}" class="bw-forgot-link" style="font-weight: 700;">{{ __('web.login') }}</a>
            </div>
            ...
            @if( (!empty($envSetting['facebook_app_id'] || config('services.facebook.client_id')) && !empty($envSetting['facebook_app_secret'] || config('services.facebook.client_secret')) && !empty($envSetting['facebook_redirect'] || config('services.facebook.redirect'))) || (!empty($envSetting['google_client_id'] || config('services.google.client_id')) && !empty($envSetting['google_client_secret'] || config('services.google.client_secret')) && !empty($envSetting['google_redirect'] || config('services.google.redirect'))) || (!empty($envSetting['linkedin_client_id'] || config('services.linkedin-openid.client_id')) && !empty($envSetting['linkedin_client_secret'] || config('services.linkedin-openid.client_secret')) && !empty(config('services.linkedin-openid.redirect'))) )
                ...
                @if (!empty($envSetting['google_client_id'] || config('services.google.client_id')) && !empty($envSetting['google_client_secret'] || config('services.google.client_secret')) && !empty($envSetting['google_redirect'] || config('services.google.redirect')))
                    <a href="{{ url('/login/google?type=2') }}" class="bw-social-btn">
                ...
                @if (!empty($envSetting['facebook_app_id'] || config('services.facebook.client_id')) && !empty($envSetting['facebook_app_secret'] || config('services.facebook.client_secret')) && !empty($envSetting['facebook_redirect'] || config('services.facebook.redirect')))
                    <a href="{{ url('/login/facebook?type=2') }}" class="bw-social-btn">
                ...
                @if (!empty($envSetting['linkedin_client_id'] || config('services.linkedin-openid.client_id')) && !empty($envSetting['linkedin_client_secret'] || config('services.linkedin-openid.client_secret')) && !empty(config('services.linkedin-openid.redirect')))
                    <a href="{{ url('/login/linkedin-openid?type=2') }}" class="bw-social-btn">
