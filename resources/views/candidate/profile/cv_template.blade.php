<div class="row resumes">
    ...
    <div class="col-md-12 mb-8 ps-7">
        @if ($user->candidateSkill->count())
            <h2 class="border-bottom border-2 border-danger pb-3 text-info fs-2x mb-5">
                <img class="text-info p-3 border rounded-circle border-info me-3"
                    src="{{ asset('assets/img/list.png') }}">
    ...
    <div class="col-md-12 mb-8 ps-7">
        @if ($candidateEducations->count())
            <h2 class="border-bottom border-2 border-danger pb-3 text-info fs-2x mb-5">
                <img class="text-info p-3 border rounded-circle border-info me-3"
                    src="{{ asset('assets/img/user-graduate.png') }}">
        ...
        <div class="row">
            @foreach ($candidateEducations as $candidateEducation)
                <div class="col-6 mt-5">
                    ...
                    <span class="text-muted fs-4">{{ $candidateEducation->year }} |
    ...
    <div class="col-md-12 mb-8 ps-7">
        @if ($candidateExperiences->count())
            <h2 class="border-bottom border-2 border-danger pb-3 text-info fs-2x mb-5">
                <img class="text-info p-3 border rounded-circle border-info me-3"
                    src="{{ asset('assets/img/briefcase.png') }}">
        ...
        <div class="row">
            @foreach ($candidateExperiences as $candidateExperience)
                <div class="col-6 mt-5">
                    ...
                    <span
                        class="text-muted fs-4">{{ \Carbon\Carbon::parse($candidateExperience->start_date)->translatedFormat('jS M, Y') }}
                        - </span>
    ...
    <div class="col-md-12 ps-7">
        @if ($user->is_online_profile_availbal)
            <h2 class="border-bottom border-2 border-danger pb-3 text-info fs-2x mb-5">
                <img class="text-info p-3 border rounded-circle border-info me-3"
                    src="{{ asset('assets/img/link.png') }}">
                {{ __('messages.candidate_profile.online_profile') }}
            </h2>
        @endif
        @if (isset($user->facebook_url))
            <a class="mb-2 fs-4 d-block" href="{{ $user->facebook_url }}" target="_blank"
                id="facebook_url">{{ $user->facebook_url }}</a>
        @endif
        @if (isset($user->twitter_url))
            <a class="mb-2 fs-4 d-block" href="{{ $user->twitter_url }}" target="_blank"
                id="twitter_url">{{ $user->twitter_url }}</a>
        @endif
        @if (isset($user->linkedin_url))
            <a class="mb-2 fs-4 d-block" href="{{ $user->linkedin_url }}" target="_blank"
                id="linkedin_url">{{ $user->linkedin_url }}</a>
        @endif
        @if (isset($user->google_plus_url))
            <a class="mb-2 fs-4 d-block" href="{{ $user->google_plus_url }}" target="_blank"
                id="google_plus_url">{{ $user->google_plus_url }}</a>
        @endif
        @if (isset($user->pinterest_url))
            <a class="mb-2 fs-4 d-block" href="{{ $user->pinterest_url }}" target="_blank"
                id="pinterest_url">{{ $user->pinterest_url }}</a>
        @endif
    </div>
</div>
