  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        displaySuccessMessage(result.message);
        setTimeout(function () {
          window.location.href = result.data.redirect_url || route('front.candidate.login');
        }, 1500);
      }
    },
    error: function error(result) {
  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        displaySuccessMessage(result.message);
        setTimeout(function () {
          window.location.href = result.data.redirect_url || route('front.employee.login');
        }, 1500);
      }
    },
    error: function error(result) {
...
function initializeJobsMegaMenu() {
  ...
  var tabs = megaDropdown.querySelectorAll('.jobs-mega-tab');
  var panels = megaDropdown.querySelectorAll('.jobs-mega-panel');
  var toggleButton = megaDropdown.querySelector('.jobs-mega-open-btn');
  var menuLinks = megaDropdown.querySelectorAll('.jobs-mega-link, .jobs-mega-simple-link');
  tabs.forEach(function (tab) {
    tab.addEventListener('click', function () {
      ...
      var targetPanel = megaDropdown.querySelector(target);
  ...
  if (toggleButton) {
    toggleButton.addEventListener('click', function (event) {
      event.preventDefault();
      var isOpen = megaDropdown.classList.toggle('is-open');
