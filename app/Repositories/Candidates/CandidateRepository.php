...
//            }
    ...
    public function updateProfile(array $input): bool
    {
        try {
            DB::beginTransaction();
            $input['dob'] = (! empty($input['dob'])) ? $input['dob'] : null;
            $input['current_salary'] = removeCommaFromNumbers($input['current_salary'] ?? null);
            $input['expected_salary'] = removeCommaFromNumbers($input['expected_salary'] ?? null);
            $input['immediate_available'] = $input['immediate_available'] ?? 1;
            ...
            $this->syncTeachingProfileRecords($user->candidate, $input);
    ...
    private function syncTeachingProfileRecords(Candidate $candidate, array $input): void
    {
        if (! empty($input['degree_level_id']) || ! empty($input['degree_title'])) {
            CandidateEducation::updateOrCreate(
                ['candidate_id' => $candidate->id],
                [
                    'degree_level_id' => $input['degree_level_id'] ?? null,
                    'degree_title' => $input['degree_title'] ?? '',
                    'country_id' => $input['education_country_id'] ?? $input['country_id'] ?? null,
                    'state_id' => $input['education_state_id'] ?? $input['state_id'] ?? null,
                    'city_id' => $input['education_city_id'] ?? $input['city_id'] ?? null,
        ...
        if (! empty($input['company']) || ! empty($input['institution_type'])) {
            CandidateExperience::updateOrCreate(
                ['candidate_id' => $candidate->id],
                [
                    'experience_title' => $input['experience_title'] ?? 'Teacher',
                    'company' => $input['company'] ?? 'Not specified',
                    'institution_type' => $input['institution_type'] ?? null,
                    'country_id' => $input['experience_country_id'] ?? $input['country_id'] ?? null,
                    'state_id' => $input['experience_state_id'] ?? $input['state_id'] ?? null,
                    'city_id' => $input['experience_city_id'] ?? $input['city_id'] ?? null,
                    'start_date' => $input['start_date'] ?? now()->startOfYear()->toDateString(),
                    'end_date' => $input['end_date'] ?? null,
                    'currently_working' => isset($input['currently_working']) ? 1 : 0,
                    'description' => $input['experience_description'] ?? null,
                ]
            );
        }
    }
